...
function parsePricingPage(html) {
  try {
    ...
    for (const pattern of modelPatterns) {
      ...
      // the price table lists models side by side: each "类别" row carries
      // one "N元" figure per model (flash first, then pro). Collect every
      // row's figures and pick the model's column.
